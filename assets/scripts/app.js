...
// const h1 = document.querySelector('.content').firstElementChild;
...
const blackDrop = document.querySelector('.black-backdrop');
...
blackDrop.addEventListener('click', toggleAddTaskForm);
...
const calculateTotal = () => {
  ...
  categoryTask.forEach((cat) => {
    console.log(cat);
  });
  totalCategoryTasks.innerHTML = `${categoryTask.length} ${
    categoryTask.length > 1 ? 'Tasks' : 'Task'
  }`;
...
const renderCategories = () => {
  ...
  categories.forEach((category) => {
    // Get all the tasks of current task
    const categoryTask = tasks.filter(
    ...
    div.addEventListener('click', () => {
      ...
      categoryImg.src = `assets/images/${selectedCategory.img}`;
      calculateTotal();
      renderTasks();
      // categoryTasks.innerHTML = `${categoryTask.length} ${
      //   categoryTasks.length > 1 ? 'Tasks' : 'Task'
      // }`;
    });
    div.innerHTML = `
              <div class="left">
              <img src="assets/images/${category.img}" alt="${
      category.title
    }" />
              <div class="content">
                <h1>${category.title}</h1>
                <p>${categoryTask.length} ${
      categoryTask.length > 1 ? 'Tasks' : 'Task'
    }</p>
              </div>
            </div>
...
const renderTasks = () => {
  ...
  if (categoryTask.length === 0) {
    ...
    categoryTask.forEach((task) => {
      ...
      // add completion functionality on click function
